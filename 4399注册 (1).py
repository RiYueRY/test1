...
代理源 = [
    "https://proxy.scdn.io/api/get_proxy.php?protocol=http&count=1"
]
...
class 高速注册机:
    ...
    def 获取单个代理(self):
        """实时从代理源拉取一个IP，返回requests可用的proxies字典。"""
        for url in 代理源:
            try:
                # 将count参数改为1，确保每次仅取一个
                单个地址 = re.sub(r"count=\d+", "count=1", url)
                resp = requests.get(单个地址, timeout=10).json()
                # 假设返回的JSON格式为 {'code': 0, 'data': [{'ip': 'x.x.x.x', 'port': 'xxxx', ...}]}
                if resp.get('code') == 0 and resp.get('data'):
                    ip = f"{resp['data'][0]['ip']}:{resp['data'][0]['port']}"
                    return {'http': f'http://{ip}', 'https': f'http://{ip}'}
                else:
                    self.安全打印(f"⚠️ 代理源 {url} 返回无有效IP: {resp}...")
            except Exception as e:
                self.安全打印(f"⚠️ 获取代理失败: {str(e)}")
        return None
